...
module.exports = {
    ...
    bundle: function(args) {
        ...
        if (!appname) {
            log.error('Must be in an application directory to run this command');
        }
        log.info('creating bundle for', appname);
        if (!exists('package.json')) {
            log.info('no package.json file found, creating.');
            var data = {
                name: process.nodester.brand,
                description: process.nodester.brand + " bundle"
            };
            fs.writeFileSync('package.json', JSON.stringify(data), encoding="utf8");
        }
        log.info('bundle is ready');
        log.plain('  npm bundle install'.white, '<package>'.yellow);
    },
    simulate: function(args) {
        ...
        nodeapi.app_info(appname, function (err, data) {
            var configData = {
                ...
                ip: '127.0.0.1',
                name: appname,
                apphome: '/tmp/nodester_' + appname
            };
            if (exists(configData.apphome)) {
                log.warn('found a existing working directory, removing it');
                exec('sudo cat ' + path.join(configData.apphome, '.nodester', 'pids', 'app.pid'), function(err, pid) {
                    pid = parseInt(pid);
                    log.warn('this chroot is running at pid:', pid);
                    if (pid > 0) {
                        try {
                    ...
                    exec('rm -rRf ' + configData.apphome, function() {
                        createApp(configData);
                    });
                });
            } else {
                createApp(configData);
...
var createApp = function(d) {
    log.info('Setting up chroot environment');
    log.info('Copying project to temporary location');
    exec('cp -R ' + process.cwd() + ' ' + d.apphome, function() {
        createSubData(d);
    });
}

var createSubData = function(d) {
    if (!exists(path.join(d.apphome, '.nodester'))) {
        log.warn('making .nodester directories');
        fs.mkdirSync(path.join(d.apphome, '.nodester'), 0777);
        fs.mkdirSync(path.join(d.apphome, '.nodester', 'logs'), 0777);
        fs.mkdirSync(path.join(d.apphome, '.nodester', 'pids'), 0777);
    }
    log.info('writing config data:', path.join(d.apphome, '.nodester', 'config.json').white);
    fs.writeFileSync(path.join(d.apphome, '.nodester', 'config.json'), JSON.stringify(d), encoding='utf8');
    fetchChroot(d);
}

var out_file = '/tmp/launch_chrooted_app.js';

var fetchChroot = function(d) {
...
var launchChroot = function(d) {
    log.info('launching chroot, needs sudo access..');
    exec('cd ' + d.apphome + ' && sudo ' + out_file + ' NODESTER_CHROOT', function(err, stdout, stderr) {
        log.info('chroot should be running:', ('http://127.0.0.1:' + d.port + '/').bold.white);
    });
};
